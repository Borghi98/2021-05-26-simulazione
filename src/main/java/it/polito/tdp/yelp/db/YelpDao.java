...
public class YelpDao {
	...
	public List<ArcoGrafo> calcolaArchi(String city, Year anno) {
		...
		try {
			...
			while(res.next()){
				result.add(new ArcoGrafo(res.getString("id1"), res.getString("id2"), res.getDouble("differenza")));
			}
			conn.close();
			return result;
	...
	public List<Business> getAllBusinessByCityAndYear(String city, Year anno){
		String sql = "SELECT * "
				+ "FROM business  "
				+ "WHERE city = ? "
				+ "AND ( SELECT COUNT(*) "
